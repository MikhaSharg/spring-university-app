...
class TeacherControllerTest {
	...
	void shouldShowListOfTeachers() throws Exception {
		...
		mockMvc.perform(get("/teachers"))
				...
				.andExpect(status().isOk()).andExpect(content().string(containsString("Teachers")))
	...
	@Test
	void shouldShowTeacherEditForm() throws Exception {
		when(controllersFacade.collectTeacherForView(teacher1.getId())).thenReturn(teacher1);
		...
		mockMvc.perform(get("/teachers/1"))
				...
				.andExpect(content().string(containsString(teacher1.getFirstName())))
				.andExpect(content().string(containsString(teacher1.getLastName())))
				.andExpect(content().string(containsString(teacher1.getAddress())))
				.andExpect(content().string(containsString(teacher1.getEmail())))
				.andExpect(content().string(containsString(teacher1.getGender())))
				.andExpect(content().string(containsString(teacher1.getAge().toString())))
				.andExpect(content().string(containsString(teacher1.getPhoneNumber().toString())));

	}

	@Test
	void shouldShowNewTeacherRegistrationForm() throws Exception {

		mockMvc.perform(get("/teachers/registerNewTeacher"))

				.andExpect(status().isOk()).andExpect(content().string(containsString("New Teacher registration")))

				.andExpect(content().string(containsString(GENDER_FEMALE)))
				.andExpect(content().string(containsString(GENDER_MALE)));
